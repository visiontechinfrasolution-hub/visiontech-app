...
# =====================================================================
        ...
        if v_file:
            ...
            with col_up: st.success("Success! ✅")
            with col_down:
                st.download_button("📥 DOWNLOAD PDF", output_pdf.getvalue(), f"Vintage_{v_file.name}", "application/pdf", key="v_down_btn_unique")
            with col_clr:
                if st.button("🧹 CLEAR ALL", key="v_clr_btn_unique"): 
                    st.rerun()
