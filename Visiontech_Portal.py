                ...
                if u_po and p_file:
                    try:
                        ...
                        if h_idx != -1:
                            ...
                            # Numeric logic for Project Wise Grouping
                            ...
                            if not df_cln.empty:
                                ...
                                for _, r in df_cln.iterrows():
                                    items.append({
                                        ...
                                        "uom": str(r.get('UOM', '')), 
                                        "qty": clean_num_fixed(r.get('Qty')), 
                                        "price": clean_num_fixed(r.get('Price')), 
                                        "amount": clean_num_fixed(r.get('Amount')), 
                                ...
                                # Project wise summary calculation
                                sums = df_cln.groupby('Project Name')['amt_tmp'].sum().reset_index()
                                summary_list = [
                                    {
                                        "po_number": str(u_po), 
                                        "project_name": str(sr['Project Name']), 
                                        "total_amount": float(sr['amt_tmp'])
                                    } for _, sr in sums.iterrows()
                                ]
                                supabase.table("po_summaries").insert(summary_list).execute()
                                
                                st.success(f"PO {u_po} Synced Successfully!")
                                st.rerun()
                    except Exception as e: 
                        st.error(f"Error: {e}")

        # --- SECTION 2: DATABASE CONTROL ---
        if st.button("🗑️ Clear All Database Data", use_container_width=True):
            try:
                # neq check bypasses Supabase security for full table deletion
                supabase.table("po_line_items").delete().neq("po_number", "DATA_CLEARED_BY_USER").execute()
                supabase.table("po_summaries").delete().neq("po_number", "DATA_CLEARED_BY_USER").execute()
                st.success("Database Cleared Successfully!")
                st.rerun()
            except Exception as e:
                st.error(f"Clear Action Failed: {e}")

        # --- SECTION 3: VISUALIZATION & SEARCH ---
        g_search = st.text_input("🔍 Search Database...", key="fin_g_search")
        f_t1, f_t2 = st.tabs(["📊 Project Summaries", "📋 Detailed Line Items"])
