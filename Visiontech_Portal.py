...
# =====================================================================
# 🟩 TAB 1: BOQ REPORT (NO CHANGES IN LOGIC)
# =====================================================================
with tab1:
    ...
    with st.form("search_form"):
        ...
        with c1: project_query = st.text_input("📁 Project No.", key="boq_p_v6")
        with c2: site_query = st.text_input("📍 Site ID", key="boq_s_v6")
        with c3: boq_query = st.text_input("📄 BOQ", key="boq_b_v6")
        with c4: dispatch_date = st.date_input("📅 Date", value=None, key="boq_d_v6")
        with c5: transporter = st.selectbox("🚚 Transporter", ["", "visiontech", "Safexpress", "Delhivery", "VRL Logistics", "TCI Express", "Gati"], key="boq_t_v6")
        with c6: tsp_partner = st.selectbox("🤝 TSP Partner", ["", "visiontech", "Partner A", "Partner B", "Partner C", "Ericsson", "Nokia"], key="boq_tsp_v6")
        with c7: 
        ...
        if has_filter:
            try:
                ...
                if response.data:
                    ...
                    df = df.fillna('').astype(str).replace(['None', 'nan', 'NULL', '<NA>'], '')
...
# =====================================================================
with tab2:
    st.markdown("<h3 style='text-align: center;'>🧾 Visiontech Infra - PO Report</h3>", unsafe_allow_html=True)
    if not st.session_state.get('po_unlocked', False):
        pwd = st.text_input("Password:", type="password", key="p_v6")
        if st.button("Unlock 🔓", key="pb_v6"):
            if pwd == "1234": st.session_state.po_unlocked = True; st.rerun()
    else:
        with st.form("po_f_v6"):
...
with tab3:
    ...
    if st.session_state.get('site_unlocked', False):
        ...
        if sub_sd:
            ...
            if res.data: st.dataframe(pd.DataFrame(res.data).fillna(''), use_container_width=True)
    else:
        if st.text_input("Password", type="password", key="s_v6") == "1234": st.session_state.site_unlocked = True; st.rerun()

# =====================================================================
# 📊 TAB 4: INDUS BASIC DATA (FIXED VERTICAL LAYOUT)
# =====================================================================
with tab4:
    st.markdown("<h3 style='text-align: center;'>📊 Indus Basic Data</h3>", unsafe_allow_html=True)
    with st.form("ind_f_v6"):
        i1, i2, i3 = st.columns(3)
        with i1: in_id = st.text_input("📍 Site ID", key="ind_id_v6")
    ...
    if sub_in:
        ...
        if res.data:
            for row in res.data:
                ...
                # Sab details ek ke niche ek (Vertical List)
                st.markdown("---")
