...
# =====================================================================
    ...
    if not st.session_state.po_unlocked:
        ...
        if submit_po_search:
            ...
            if search_po:
                try: query = query.eq("PO Number", int(search_po.strip())); has_filter = True
                except: st.error("PO Number must be numeric")
            if search_shipment:
                try: query = query.eq("Shipment Number", int(search_shipment.strip())); has_filter = True
                except: st.error("Shipment Number must be numeric")
            if search_receipt:
                try: query = query.eq("Receipt Number", int(search_receipt.strip())); has_filter = True
                except: st.error("Receipt Number must be numeric")
            
            if has_filter:
                res = query.execute()
                if res.data: st.dataframe(pd.DataFrame(res.data), use_container_width=True, hide_index=True)
                else: st.warning("❌ No PO Data found")
...
# =====================================================================
    ...
    if not st.session_state.site_unlocked:
        ...
        if submit_site_search:
            ...
            res = q.execute()
            if res.data: st.dataframe(pd.DataFrame(res.data), use_container_width=True, hide_index=True)
            else: st.warning("❌ No Site Data found")

# =====================================================================
# 📊 PAGE 4: INDUS BASIC DATA (PLAIN FORMAT & FSE FIX)
# =====================================================================
    ...
    if submit_indus:
        if s_id or s_nm or s_cl:
            ...
            if res.data:
                for row in res.data:
                    # FSE Handle (Checking both space and no-space)
