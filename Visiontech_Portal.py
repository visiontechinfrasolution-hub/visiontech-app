...
with tab1:
    ...
    if 'boq_df' in st.session_state:
                ...
                if ind_res.data:
                    ...
                    def call_btn(label, name, num):
                        if num and str(num).strip() not in ['-', '', 'None']:
                            return f'{label}: **{name}** ({num}) <a href="tel:{num}"><button style="background-color:#007bff;color:white;border:none;padding:2px 8px;border-radius:4px;cursor:pointer;font-weight:bold;">📞 Call</button></a>'
                        return f'{label}: **{name}** (-)'
                    v1, v2 = st.columns(2)
                    with v1:
                    ...
                    st.session_state['wa_indus_data'] = f"\n📍 *INDUS SITE DATA*\n*Area Name* :- {row_in.get('Area Name','-')}\n*Tech Name* :- {row_in.get('Tech Name','-')} ({row_in.get('Tech Number','-')})\n*FSE* :- {row_in.get('FSE','-')} ({row_in.get('FSE Number','-')})\n*AOM Name* :- {row_in.get('AOM Name','-')} ({row_in.get('AOM Number','-')})\n"
            except: pass
        ...
        with btn_col2:
            wa_msg = f"📦 *BOQ REPORT: {p_val}*\n*Site ID* - {s_id}\n\n" + st.session_state.get('wa_indus_data', "")
...
# =====================================================================
# 📊 TAB 4: INDUS BASIC DATA (RESTORED SEARCH BOX)
# =====================================================================
with tab4:
