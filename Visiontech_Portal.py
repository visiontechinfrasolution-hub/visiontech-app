...
# =====================================================================
# 🟩 TAB 1: BOQ REPORT (SAB KUCH ORIGINAL - NO NULLS)
# =====================================================================
with tab1:
    ...
    with st.form("search_form"):
        ...
        with c1: project_query = st.text_input("📁 Project No.", key="boq_p_final_v4")
        with c2: site_query = st.text_input("📍 Site ID", key="boq_s_final_v4")
        with c3: boq_query = st.text_input("📄 BOQ", key="boq_b_final_v4")
        with c4: dispatch_date = st.date_input("📅 Date", value=None, key="boq_d_final_v4")
        with c5: transporter = st.selectbox("🚚 Transporter", ["", "visiontech", "Safexpress", "Delhivery", "VRL Logistics", "TCI Express", "Gati"], key="boq_t_final_v4")
        with c6: tsp_partner = st.selectbox("🤝 TSP Partner", ["", "visiontech", "Partner A", "Partner B", "Partner C", "Ericsson", "Nokia"], key="boq_tsp_final_v4")
        with c7: 
    ...
    with r2: boq_date_input = st.date_input("Select Date", value=None, label_visibility="collapsed", key="boq_q_d_final_v4")
...
# =====================================================================
# 🟦 TAB 2: PO REPORT (SAB KUCH ORIGINAL - NO NULLS)
# =====================================================================
with tab2:
    ...
    if not st.session_state.get('po_unlocked', False):
        pwd = st.text_input("Enter Password:", type="password", key="p_pwd_v4")
        if st.button("Unlock 🔓", key="p_u_v4"):
            if pwd == "1234": st.session_state.po_unlocked = True; st.rerun()
    else:
        with st.form("po_form_v4"):
