...
# =====================================================================
# 🟩 TAB 1: BOQ REPORT (ORIGINAL SEARCH BOXES & BUTTONS)
# =====================================================================
with tab1:
...
# =====================================================================
# 🟦 TAB 2: PO REPORT (PO NUMBER ON LEFT & CENTERED SUMMARY)
# =====================================================================
with tab2:
    ...
    if not st.session_state.get('po_unlocked', False):
        pwd = st.text_input("Enter Password:", type="password", key="p_pwd")
        if st.button("Unlock 🔓", key="p_u_b"):
    ...
    if st.session_state.get('site_unlocked', False):
        ...
        if sub_sd:
            ...
            if res.data: 
                ...
                wa_msg = f"🏗️ *SITE DETAIL REPORT*\n📍 *ID:* {site_id}\n🚀 _Visiontech_"
                st.markdown(f'<a href="whatsapp://send?text={urllib.parse.quote(wa_msg)}"><button style="background-color: #25D366; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; font-weight: bold;">📲 Share to Group</button></a>', unsafe_allow_html=True)
    else:
        pwd = st.text_input("Enter Password:", type="password", key="s_pwd_t")
        if st.button("Unlock Site 🔓", key="s_u_b"):
            if pwd == "1234": st.session_state.site_unlocked = True; st.rerun()

# =====================================================================
# 📊 TAB 4: INDUS BASIC DATA (PLANE FORMAT)
# =====================================================================
with tab4:
