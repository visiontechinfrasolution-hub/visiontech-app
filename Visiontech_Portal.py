...
with tab2:
    ...
    if not st.session_state.po_unlocked:
        ...
        with st.form("po_form"):
            ...
            with col2: s_ship = st.text_input("🚚 Shipment Number")
            with col3: s_rec = st.text_input("🧾 Receipt Number")
    ...
    if sub_in:
        ...
        if res.data:
            for row in res.data:
                ...
                st.markdown(f"--- \n**Site ID** :- {row.get('Site ID')} | **Site Name** :- {row.get('Site Name')}\n**FSE** :- {fse_name} | **Lat-Long** :- {row.get('Lat')} {row.get('Long')}")
                wa_msg = f"*VISPL REPORT*\nSite: {row.get('Site ID')}\nFSE: {fse_name}"
