    ...
    if st.button("🚀 Calculate Best Route", use_container_width=True):
        if not start_coords or not end_coords or not st.session_state.route_list:
            ...
            try:
                ...
                def get_lat_lon(loc_input):
                    if ',' in loc_input and any(char.isdigit() for char in loc_input):
                        ...
                        if location:
                            ...
                            raise ValueError(f"Location '{loc_input}' nahi mili.")
                ...
                # Optimized logic: Nearest Neighbor with NoneType Check
                while unvisited:
                    # Sirf unhi sites ko check karega jinka Lat/Long available aur valid hai
                ...
                if unvisited:
                    st.warning(f"Kuch sites skip hui (Missing Lat/Long): {', '.join([s['Site ID'] for s in unvisited])}")
