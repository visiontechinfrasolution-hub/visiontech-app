...
with tab1:
    ...
    if submit_search or stn_pending_btn or gen_new_boq or update_click:
        ...
        if update_click:
            try:
                sd_res = supabase.table("Site Detail").select("*").limit(1000).execute()
                if sd_res.data:
                    df_sd = pd.DataFrame(sd_res.data)
                    possible_cols = ["Project Number", "Project ID", "PROJECT NUMBER"]
                    target_col = next((c for c in possible_cols if c in df_sd.columns), None)
                    
                    if target_col:
                        p_list = df_sd[target_col].dropna().astype(str).unique().tolist()
                        ...
                        st.error("Site Detail table columns match nahi ho rahe.")
            except:
                st.error("Supabase API Error - Check Table Names")
...
# ... baaki tabs purane code jaise hi rahenge
